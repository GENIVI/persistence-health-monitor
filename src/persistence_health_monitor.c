#ifndef PERSISTENCE_HEALTH_MONITOR_H
#define PERSISTENCE_HEALTH_MONITOR_H
...
******************************************************************************/
 /**
 * @file           persistence_health_monitor.h
...
void printfUsage()
{
   ...
   printf("   -d start the PCM as deamon\n");
...
int main(int argc, char *argv[])
{
	...
	if(startDaemon == 1)
	{
		DLT_LOG(phmContext, DLT_LOG_INFO, DLT_STRING("Persistence health monitor - demonize") );
		// make a daemon process
		if(daemon(1, 1) == -1)
		{
			DLT_LOG(phmContext, DLT_LOG_ERROR, DLT_STRING("demonize failed: Cannot detach!") );
			_exit(-1);
		}
	}
...
#endif /* PERSISTENCE_HEALTH_MONITOR_H */
